  ...
  void uploadStatus({
    ...
    try {
      ...
      var statusSnapshot = await firestore
          .collection('chatappStatus')
          .where(
            'uid',
            isEqualTo: auth.currentUser!.uid,
          )
          // bunun yerine hepsini çektikten sonra
          // il ile kontrol edeceğiz
          // .where(
          //   'createdAt',
      ...
      await firestore.collection('chatappStatus').doc(statusId).set(
            status.toMap(),
          );
    } catch (e) {
      showSnackBar(
